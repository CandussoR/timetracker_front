<template>
    <div v-if="props.task && (subtaskCanBeNull || (props.view != 'timerForm'))">
        <label v-if="subtaskCanBeNull || (props.view != 'timerFom')" for="subtask" class="sr-only">Subtask name (optional) : </label>
...
const subtaskCanBeNull = computed(() => subtasks.value.includes(null) || subtasks.value.length == 0);
